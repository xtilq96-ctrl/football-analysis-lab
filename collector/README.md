...
- V2 可融合近 10 场、主客场攻防、休息天数、伤停停赛和临场首发
...
V1 回退模式的比分与总进球采用市场概率校准的独立泊松模型（`v1-market-poisson`）。天气和球队近期 xG 暂未纳入；所有模型都必须通过长期赛果回测后才能评估真实预测能力。

配置有效的 API-Football 订阅后，模型升级为 `v2-market-fundamentals`。服务器从 `/etc/football-ai/collector.env` 读取 `API_FOOTBALL_KEY`。球队匹配不达安全阈值或接口不可用时自动退回 V1，不会用错误基本面强行计算。详细规则见 `docs/V2_FUNDAMENTALS_ENGINE.md`。
