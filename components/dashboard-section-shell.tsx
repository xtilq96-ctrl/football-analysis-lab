  ...
  return (
    <main className="min-h-screen bg-background pb-20 text-foreground lg:pb-0">
      ...
      <div className="mx-auto grid max-w-[1440px] gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:px-8 lg:py-8">
        <aside className="hidden lg:block">
          <nav className="sticky top-24 space-y-1" aria-label="主要导航">
            {navigation.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${active === href ? 'bg-lime-300 font-medium text-[#07110d]' : 'text-white/50 hover:bg-white/5 hover:text-white/85'}`}><Icon className="h-[17px] w-[17px]" />{label}</Link>)}
          </nav>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 border-t border-white/8 bg-[#07110d]/95 px-1 pb-[max(.5rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl lg:hidden" aria-label="移动端导航">
        {navigation.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={`flex flex-col items-center gap-1 text-[11px] ${active === href ? 'text-lime-300' : 'text-white/40'}`}><Icon className="h-5 w-5" />{label.replace('中心', '').replace('模型', '')}</Link>)}
      </nav>
    </main>
  );
}
