...
export default async function SystemPage() {
  ...
  return <DashboardSectionShell active="/system" title="数据与任务" description="查看云端采集、历史回填、模型生成和赛果核对是否正常。">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <SectionCard title="分析时限"><Rows rows={[["分析已生成", `${operations?.analysis.readyMatches ?? data.matches.length}/${operations?.analysis.totalMatches ?? data.matches.length}场`],["已经锁定", `${operations?.analysis.lockedMatches ?? 0}场`],["下一次最终分析", nextAnalysis],["最终分析", "开赛前20分钟或21:15"],["锁定规则", "开赛前10分钟或21:30"]]} /></SectionCard>
      ...
      <SectionCard title="分时刷新调度"><Rows rows={[["赛程匹配", apiUsage?.schedule.fixtures ?? "每6小时按日期批量刷新"],["伤停停赛", apiUsage?.schedule.injuries ?? "09:00/15:00/18:30/20:30"],["临场首发", apiUsage?.schedule.lineups ?? "赛前180/90/45/20分钟"],["出票截止", "21:30后不再改变当天推荐"],["已确认首发", "停止重复查询"]]} /></SectionCard>
      ...
      <SectionCard title="自动复盘"><Rows rows={[["已核对赛果", `${data.performance.settledMatches}场`],["已核对组合", `${data.performance.settledCombinations ?? data.performance.settledTwoLegs}组`],["动态串数", "不推荐 / 2 / 3 / 4串1"],["预测锁定", "开赛前10分钟或21:30"],["截止后用途", "只收集赛果、复盘和训练"],["运行状态", data.error ? "数据重试中" : "正常"]]} /></SectionCard>
...
function trainingDecision(value: 'collecting' | 'hold' | 'promote' | 'rollback') { return ({ collecting: '继续收集样本', hold: '未通过，暂不升级', promote: '通过验证，已启用', rollback: '出现退化，已回退' } as const)[value]; }
