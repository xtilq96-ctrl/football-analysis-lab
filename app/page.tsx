    ...
    <main className="football-shell min-h-screen pb-20 text-foreground lg:pb-0">
      ...
      <div className="mx-auto grid max-w-[1440px] gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)_300px] lg:px-8 lg:py-8">
        ...
        <section className="min-w-0">
          ...
          <div className="pitch-overview mb-5 grid grid-cols-3 overflow-hidden rounded-2xl border border-lime-200/15">
            <OverviewMetric icon={<Goal />} label="今日场次" value={String(data.matches.length)} suffix="场" />
            <OverviewMetric icon={<Sparkles />} label="模型已分析" value={String(predicted)} suffix="场" />
            <OverviewMetric icon={<ShieldCheck />} label="基本面覆盖" value={String(fundamentalCompleteness)} suffix="%" />
          </div>
    ...
    <Card className="match-card group overflow-hidden border-white/8 bg-[#0a1711]/72 py-0 shadow-none transition-all duration-300 hover:-translate-y-0.5 hover:border-lime-200/20 hover:bg-[#0c1b14]/82">
      <CardContent className="p-5 sm:p-6">
        ...
        <div className="mt-5 grid items-center gap-5 sm:grid-cols-[minmax(170px,.8fr)_minmax(280px,1.2fr)_auto]">
          <div><div className="flex items-center gap-3 text-lg font-semibold tracking-tight"><span>{match.home}</span><span className="text-sm font-normal text-white/25">vs</span><span>{match.away}</span></div><p className="mt-1 text-xs text-white/35">{match.goals} · {match.confidence === null ? '等待固定奖' : `最高概率 ${match.confidence}%`}{match.dataQuality ? ` · 可信度 ${match.dataQuality.adjustedConfidence ?? '—'}%（${match.dataQuality.grade}级）` : ''}</p></div>
          <div>
          ...
          <a href={`/schedule#match-${match.id}`} className="inline-flex h-8 items-center justify-self-start rounded-md px-3 text-sm text-white/60 hover:bg-white/7 hover:text-white sm:justify-self-end">分析摘要 <ChevronRight className="ml-1 h-4 w-4" /></a>
        </div>
        ...
        {match.predictedScore && <div className="mt-3 rounded-xl border border-violet-400/12 bg-violet-400/[.055] p-3">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-1.5"><span className="mr-1 text-xs text-white/35">比分前三候选</span>{match.scoreProbabilities.slice(0, 3).map((score, index) => <Badge key={score.score} variant="outline" className={index === 0 ? 'border-violet-300/25 bg-violet-300/10 text-violet-100' : 'border-white/10 text-white/55'}>{index + 1}. {score.score} · {score.probability}%</Badge>)}</div>
        </div>}
        {match.dataQuality && <div className="mt-3 rounded-xl border border-white/7 bg-black/10 p-3 text-xs"><div className="flex flex-wrap items-center justify-between gap-2"><span className={match.dataQuality.grade === 'A' ? 'text-emerald-200' : match.dataQuality.grade === 'B' ? 'text-lime-200' : match.dataQuality.grade === 'C' ? 'text-amber-200' : 'text-orange-200'}>数据完整度 {match.dataQuality.score}/100 · {match.dataQuality.grade}级 {match.dataQuality.label}</span><span className="text-white/35">{match.dataQuality.recommendationEligible ? '可进入组合筛选' : '不进入组合推荐'}</span></div>{match.dataQuality.missing.length > 0 && <p className="mt-1 text-white/35">待补：{match.dataQuality.missing.join('、')}</p>}</div>}
...
function RecommendationPanel({ recommendations, decision }: { recommendations: DashboardRecommendation[]; decision: RecommendationDecision | null }) {
  return (
    <Card className="champion-panel relative mb-5 overflow-hidden border-lime-300/20 bg-[linear-gradient(135deg,rgba(190,242,100,.12),rgba(255,255,255,.025))] shadow-none">
      <CardHeader className="pb-3">
        <CardTitle className="flex flex-wrap items-center justify-between gap-2 text-base font-medium">
          <span className="flex items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-full border border-lime-200/25 bg-lime-200/10"><Medal className="h-4 w-4 text-lime-200" /></span>自动组合决策 · {recommendations[0]?.type ?? '今日不推荐'}</span>
      ...
      <CardContent>
        {recommendations.length ? <div className="grid gap-3 xl:grid-cols-3">{recommendations.map((recommendation, index) => (
          <div key={recommendation.legs.map((leg) => leg.matchId).join('-')} className="rounded-xl border border-white/8 bg-black/10 p-3">
            ...
            <div className="space-y-2">{recommendation.legs.map((leg) => <div key={leg.matchId} className="text-sm"><p className="text-white/75">{leg.officialNumber} · {leg.pick} <span className="text-xs text-white/35">@ {leg.odds}</span></p><p className="truncate text-xs text-white/35">{leg.home} vs {leg.away} · 概率 {leg.probability}% · 可信 {leg.decisionConfidence ?? '—'}% / {leg.dataQualityGrade ?? '—'}级</p></div>)}</div>
...
function NavItem({ href, icon, label, active = false }: { href: string; icon: React.ReactNode; label: string; active?: boolean }) {
  return <a href={href} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition ${active ? 'bg-lime-300 font-medium text-[#07110d]' : 'text-white/50 hover:bg-white/5 hover:text-white/85'}`}><span className="[&>svg]:h-[17px] [&>svg]:w-[17px]">{icon}</span>{label}</a>;
}

function MobileNav({ href, icon, label, active = false }: { href: string; icon: React.ReactNode; label: string; active?: boolean }) {
  return <a href={href} className={`flex flex-col items-center gap-1 text-[11px] ${active ? 'text-lime-300' : 'text-white/40'}`}><span className="[&>svg]:h-5 [&>svg]:w-5">{icon}</span>{label}</a>;
}
